...
export default function AudioRecording() {
  ...
  return (
    ...
    >
      <Stack spacing={1} sx={{ mt: 2, display: 'flex', alignItems: 'center' }}>
        <Typography
          sx={{
        ...
        <Typography sx={{ color: '#7c7c80', fontSize: 20, fontWeight: 400 }}>
        ...
        {bars.map((_, index) => (
          <Box
            ...
            sx={{
              width: 4,
              borderRadius: 999,
              background: index % 2 === 0 ? '#7ea6ff' : '#cddcff',
              height: isRecording ? `${20 + ((index * 13) % 90)}px` : '12px',
      ...
      {audioBlob && !isRecording && (
        <Box sx={{ width: '90%', maxWidth: 420, mb: 3 }}>
      ...
      {analysisResult && uploadStatus === 'success' && (
        ...
        >
          <Typography sx={{ fontSize: '1.1rem', fontWeight: 600, color: '#1c1c1e', mb: 2 }}>
            Análise do áudio
          </Typography>

          {analysisResult.transcription && (
            <Box sx={{ mb: 2 }}>
              <Typography sx={{ fontSize: '0.9rem', fontWeight: 600, color: '#667eea', mb: 1 }}>
                Transcrição
              </Typography>
              <Typography sx={{ fontSize: '0.9rem', color: '#4a4a4a', lineHeight: 1.6 }}>
                {analysisResult.transcription}
              </Typography>
            </Box>
          )}
          ...
          {analysisResult.emotion && (
            <Box sx={{ mb: 2 }}>
              ...
              <Typography sx={{ fontSize: '0.9rem', color: '#4a4a4a' }}>
                {analysisResult.emotion} (confiança:{' '}
                {analysisResult.emotion_confidence?.toFixed(2)})
              </Typography>
            </Box>
          )}
      ...
      >
        ...
        >
          {isRecording ? (
            <StopRoundedIcon sx={{ color: 'white', fontSize: 42 }} />
          ) : (
            <PlayArrowRoundedIcon sx={{ color: 'white', fontSize: 46, ml: '2px' }} />
          )}
        </IconButton>
